...
# Create the RG
az group create -n $rgName -l northcentralus --tags "Environment=$envName" "CreatedBy=$createBy" "CreateDt=$createDt" "Owner=$appOwner" "App=$appName" "Repo=$repoName" "Location=$locName"
...
docker build -f ./myApp/Dockerfile -t myapp:v0.1 ./myapp --build-arg tag=v0.1
docker run -p 5401:80 -d myapp:v0.1
az acr login -n vsl22acr

# Docker build on ACR & push image
az acr build --image vsl22acr.azurecr.io/myapp:v0.1 --registry vsl22acr -f ./myApp/Dockerfile ./myapp --build-arg tag=v0.1
